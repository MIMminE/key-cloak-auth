// sso-oauth2-server-2: Minimal Express server demonstrating Authorization Code + PKCE with Keycloak
...
const CLIENT_ID = process.env.KEYCLOAK_CLIENT_ID || 'sso-client-2';
const CLIENT_SECRET = process.env.KEYCLOAK_CLIENT_SECRET || '';
const APP_BASE_URL = process.env.APP_BASE_URL || `http://localhost:${port}`;
const SESSION_SECRET = process.env.SESSION_SECRET || 'keyboard cat 2';

app.use(session({ secret: SESSION_SECRET, resave: false, saveUninitialized: true }));

app.get('/', (req, res) => {
  res.send(`<h2>SSO Server 2 (port ${port})</h2>
...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    res.send(`<h2>Logged in (Server 2)</h2>
...
app.get('/profile', async (req, res) => {
  ...
  try {
    ...
    res.send(`<h2>User info (Server 2)</h2><pre>${JSON.stringify(resp.data, null, 2)}</pre>`);
  } catch (err) {
    console.error(err.response ? err.response.data : err.message);
...
app.listen(port, () => console.log(`SSO Server 2 listening on ${port}`));
